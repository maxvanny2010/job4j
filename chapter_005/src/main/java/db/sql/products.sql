...
select name_type, sum(amount)
from product p,
     type_product t
where p.type_id_fk = t.id
group by name_type;
--6 // query all products with type 'milk' and 'protein'.
select name_type, id_product, name_product, price, amount, expired_date
from product p
...
select name_type
from type_product t
         inner join product p on p.type_id_fk = t.id and p.amount < 10;
SELECT t.name_type
FROM product AS p
         LEFT JOIN type_product AS t ON t.id = p.type_id_fk
